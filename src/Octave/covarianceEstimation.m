...
% call solver
[b,~,exitflag] = fminsearchbnd(objectfun,b0,lb,ub);
...
switch type
    case 'bounded'
        ...
        hold on
        fplot(@(h) funnugget(b) + func(b,h),[0 b(1)], 'Color','r')
        fplot(@(h) funnugget(b) + b(2),     [b(1) max(h)], 'Color','r')
    ...
    case 'unbounded'
        plot(h,gammaexp,'bs','MarkerSize',10, 'MarkerFaceColor','c','LineWidth', 2);
        hold on
        fplot(@(h) funnugget(b) + func(b,h),[0 max(h)],'Color','r')
